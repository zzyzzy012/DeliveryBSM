/**
 * 工具函数封装
 *
 */
import dayjs from 'dayjs'
import type { Menu } from '@/types/api'

// 格式化金额
export const formatMoney = (num: number | string) => {
  const n = parseFloat(num.toString())
  return n.toLocaleString('zn-CN', { style: 'currency', currency: 'CNY' })
}

// 格式化数字
export const formatNumber = (num: number | string) => {
  if (!num) return 0
  const a = num.toString()
  if (a.indexOf('.') > -1) return a.replace(/(\d)(?=(\d{3})+\.)/g, '$1,')
  return a.replace(/(\d)(?=(\d{3})+$)/g, '$1,')
}

// 格式化日期
export const formatDate = (date?: string | number | Date, format: string = 'YYYY-MM-DD HH:mm:ss') => {
  const currD = date ? date : Date.now()
  return dayjs(currD).format(format)
}

// 格式化用户状态
export const formatUserState = (state: number) => {
  if (state === 1) {
    return '在职'
  } else if (state === 2) {
    return '离职'
  } else {
    return '试用期'
  }
}

// 获取页面路径
export const getMenuPath = (list: Menu.MenuItem[]): string[] => {
  return list.reduce((result: string[], item: Menu.MenuItem) => {
    return result.concat(Array.isArray(item.children) && !item.buttons ? getMenuPath(item.children) : item.path + '')
  }, [])
}

// 递归获取路由对象

export const searchRoute: any = (path: string, routes: any = []) => {
  for (const item of routes) {
    if (item.path === path) return item
    if (item.children) {
      const result = searchRoute(path, item.children)
      if (result) return result
    }
  }
  return ''
}

/**
 * 手机号加密
 * @example
 * 17611000011 => 176****0011
 */
export const formateMobile = (mobile?: number) => {
  if (!mobile) return '-'
  const phone = mobile.toString()
  return phone.replace(/(\d{3})\d*(\d{4})/, '$1****$2')
}

/**
 * 递归查找树的路径
 */
export const findTreeNode = (tree: Menu.MenuItem[], pathName: string, path: string[]): string[] => {
  if (!tree) return []
  for (const data of tree) {
    path.push(data.menuName)
    if (data.path === pathName) return path
    if (data.children?.length) {
      const list = findTreeNode(data.children, pathName, path)
      if (list?.length) return list
    }
    path.pop()
  }
  return []
}
